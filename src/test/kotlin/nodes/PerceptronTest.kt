...
class PerceptronTest : Spek({
	describe("Perceptron") {
		...
		it("should have default bias of -2") {
			assertThat( unit.bias ).isEqualTo( -2.0 )
		}

		on("being constructed with inputs and weight") {
			val givenInputs = listOf(6.0, 7.0, 8.0)
			val givenWeights = listOf(3.0, 4.0, 5.0)
			unit = Perceptron(-4.0, givenInputs, givenWeights)

			it("should set the fields as expected") {
				assertThat(unit.bias).isEqualTo(-4.0)
				assertThat(unit.inputs).isEqualTo(givenInputs)
				assertThat(unit.weights).isEqualTo(givenWeights)
			}
		}
		...
		on("being constructed with values that should not make it fire") {
			val givenInputs = listOf(1.0, 1.0, 1.0)
			val givenWeights = listOf(1.0, 1.0, 1.0)
			unit = Perceptron(-4.0, givenInputs, givenWeights)
